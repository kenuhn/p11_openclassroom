...
const Etoiles = (props) => {
    ...
    for (let i = 0; i < etoiles; i++) {
        ...
        elEtoilesColore.push(<FontAwesomeIcon key={nameKey}icon={faStar} style={{color: "#FF6060"}}/> )   
    }
    for (let i = 0; i < 5 - etoiles; i++) {
        let nameKey = "etoilesVides " +i;
        elEtoileGris.push(<FontAwesomeIcon key={nameKey} icon={faStar} style={{color: "#E3E3E3"}}/> )   
   }
    return (
